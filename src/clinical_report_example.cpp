...
bool ClinicalReportExample::create_clinical_report_pdf(const Patient &patient, const ReferringDoctor &doctor,
                                                       const std::string &output_pdf_path) const {
  // Step 1: Validate minimal required payload before allocating libHaru objects.
  if (output_pdf_path.empty() || patient.full_name.empty() || patient.patient_id.empty() || doctor.name.empty()) {
    return false;
  }
  ...
  draw_text(page, regular_font, 12.0F, left, height - 192.0F, "Age: " + std::to_string(patient.age) + " Years");
  ...
  y -= 22.0F;
  draw_text(page, regular_font, 12.0F, left + 10.0F, y, "• Both kidneys are visualized and normal in size, shape and echotexture.");
  y -= 20.0F;
  draw_text(page, regular_font, 12.0F, left + 10.0F, y, "• Right kidney measures 10.0 x 3.2 cm. Left kidney measures 9.7 x 4.2 cm.");
  y -= 20.0F;
  draw_text(page, regular_font, 12.0F, left + 10.0F, y, "• No calculus, hydronephrosis, or focal lesion seen.");

  y -= 36.0F;
  draw_text(page, bold_font, 13.0F, left, y, "URINARY BLADDER & UTERUS");
  y -= 22.0F;
  draw_text(page, regular_font, 12.0F, left + 10.0F, y, "• Urinary bladder is distended, lumen echo-free.");
  y -= 20.0F;
  draw_text(page, regular_font, 12.0F, left + 10.0F, y, "• Uterus appears normal in size and echotexture. Bilateral adnexa clear.");

  y -= 34.0F;
  draw_text(page, bold_font, 13.0F, left, y, "IMPRESSION");
  y -= 22.0F;
  draw_text(page, bold_font, 13.0F, left, y, "NO SIGNIFICANT ABNORMALITY DETECTED");
  ...
  HPDF_Page_Stroke(page);
  draw_text(page, regular_font, 11.0F, box_x + 56.0F, box_y + box_size / 2.0F, "Ultrasound image placeholder");
