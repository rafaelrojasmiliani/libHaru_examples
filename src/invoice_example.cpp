...
void draw_text(HPDF_Page page, HPDF_Font font, const float size, const float x, const float y, const std::string &text) {
...
void draw_label_value(HPDF_Page page, HPDF_Font label_font, HPDF_Font value_font, const float x_label, const float x_value, const float y,
                      const std::string &label, const std::string &value) {
  draw_text(page, label_font, 10.5F, x_label, y, label);
  draw_text(page, value_font, 10.5F, x_value, y, value);
}

void draw_line(HPDF_Page page, const float x1, const float y1, const float x2, const float y2, const float line_width, const float red,
...
bool InvoiceExample::createInvoidcw(const Provider &provider, const Client &client, const std::vector<Item> &items,
                                    const std::string &output_pdf_path) const {
  // Step 1: Validate semantic inputs before touching libHaru resources.
  if (output_pdf_path.empty() || provider.name.empty() || client.name.empty() || items.empty()) {
    return false;
  }
  ...
  draw_label_value(page, bold_font, regular_font, col3_x, col3_x + 100.0F, block_top, "INVOICE #", invoice_number);
  draw_label_value(page, bold_font, regular_font, col3_x, col3_x + 100.0F, block_top - 20.0F, "INVOICE DATE", "10/02/2026");
  draw_label_value(page, bold_font, regular_font, col3_x, col3_x + 100.0F, block_top - 40.0F, "P.O.#", "PO-4821");
  draw_label_value(page, bold_font, regular_font, col3_x, col3_x + 100.0F, block_top - 60.0F, "DUE DATE", "25/02/2026");

  // Step 5: Build the items table structure (header separators + headings).
  const float table_top = page_height - 330.0F;
  draw_line(page, margin_left, table_top, margin_right, table_top, 1.5F, accent_r, accent_g, accent_b);
  ...
  draw_line(page, margin_left, table_top - 28.0F, margin_right, table_top - 28.0F, 1.5F, accent_r, accent_g, accent_b);
  ...
  draw_text(page, regular_font, 11.0F, margin_left + 560.0F, totals_y - 22.0F, money_string(tax));
  ...
  draw_line(page, margin_left + 300.0F, footer_y - 8.0F, margin_left + 300.0F, footer_y + 110.0F, 1.0F, navy_r, navy_g, navy_b);

  HPDF_Page_SetRGBFill(page, accent_r, accent_g, accent_b);
  draw_text(page, bold_font, 15.0F, margin_left + 310.0F, footer_y + 95.0F, "TERMS & CONDITIONS");

  HPDF_Page_SetRGBFill(page, 0.05F, 0.07F, 0.12F);
  draw_text(page, regular_font, 11.0F, margin_left + 310.0F, footer_y + 62.0F, "Payment is due within 15 days");
  draw_text(page, regular_font, 11.0F, margin_left + 310.0F, footer_y + 38.0F, "Name of Bank");
  draw_text(page, regular_font, 11.0F, margin_left + 310.0F, footer_y + 20.0F, "Account number: 1234567890");
  draw_text(page, regular_font, 11.0F, margin_left + 310.0F, footer_y + 2.0F, "Routing: 098765432");
